...
const processGoogleDriveLink = (url) => {
  // Lista de padrões de links do Google Drive para identificar e extrair o ID
  const drivePatterns = [
    /https:\/\/drive\.google\.com\/file\/d\/([a-zA-Z0-9_-]+)\/view/, // Formato moderno: https://drive.google.com/file/d/ID/view
    /https:\/\/drive\.google\.com\/open\?id=([a-zA-Z0-9_-]+)/,      // Formato antigo: https://drive.google.com/open?id=ID
    /https:\/\/drive\.google\.com\/uc\?export=view&id=([a-zA-Z0-9_-]+)/ // Outro formato: https://drive.google.com/uc?export=view&id=ID
  ];

  // Verifica cada padrão para encontrar o ID
  for (const pattern of drivePatterns) {
    const match = url.match(pattern);
    if (match && match[1]) {
      const fileId = match[1]; // O ID está no primeiro grupo capturado pela regex
      console.log(`Link do Google Drive detectado. ID extraído: ${fileId}`);
      return `https://drive.google.com/thumbnail?id=${fileId}`; // Retorna o formato correto
    }
  }

  // Se não for um link do Google Drive, retorna o URL original
...
const saveToIndexedDB = (data) => {
  ...
  const dataToSave = {
    ...
    mediumPhoto: data.mediumPhoto || '', // Usa o link processado ou vazio
  ...
  if (currentEditId !== null) {
    dataToSave.id = currentEditId; // Mantém o ID para edição
  }
...
document.getElementById('convertBtn').addEventListener('click', () => {
  ...
  let inputMediumPhoto = document.getElementById('inputMediumPhoto')?.value.trim() || ''; // Pode ser vazio
  ...
  if (inputMediumPhoto) {
    inputMediumPhoto = processGoogleDriveLink(inputMediumPhoto); // Converte para o formato thumbnail
  }
  ...
  const dataToSave = {
    ...
    author: inputAuthor,
    about: inputAbout,
    mediumPhoto: inputMediumPhoto, // Link processado
  ...
  saveToIndexedDB(dataToSave); // Salva no IndexedDB
});
